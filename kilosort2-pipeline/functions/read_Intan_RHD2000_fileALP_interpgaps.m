function [intandata, gapInfo] = read_Intan_RHD2000_fileALP_interpgaps(filename)
...
%
% ALP updated Feb 2020 to supress excess strings to the command line, and
% to output the extracted data to a structure as opposed to moving it to
% the workspace. 
% ALP 3/16/2021 adding support for data files with gaps in them. use linear
% interpolation to correct the gaps. See code for further details 
...
%     fprintf(1, 'Parsing data...\n');
    ...
    if (num_gaps == 0)
        ...
        isGap = 0; 
        gapInfo.isGap = isGap; 
    else %%% all this written ALP march 2021 to interp over gaps
        fprintf('Warning: %d gaps in timestamp data found.  Time scale will not be uniform!\n', ...
            num_gaps);
        isGap = 1; 
        
        %%% ALP 3/11/2021 I have seen two different scenarios of gaps
        %-------------------------- brainstorming ---------------
        %   scenario 1: t_amplifier = [1 2 3 7 8 9 10]; 
        %       %this will show up as 1 gap
        %       num_gaps = 1;
        %       iGap = 3; 
        %       nMissingSamp = (7-3)-1 = 3;
        %       xVals = [t_amplifier(iGap:iGap+1];
        %       yVals = amplifier_data(ch,iGap:iGap+1);
        %       wantVals = t_amplifier(iGap)+1:1:t_amplifier(iGap+1)-1; 
        %       interpVals = interp1(xVals, yVals, wantVals); 
        %       notes = [num2str(num_gaps) ' gaps of length '
        %       num2str(nMissingSamp) ' samples. Linear interpolation
        %       starting at ind ' num2str(iGap)]; 
        %       
        %   scenario 2: t_amplifier = [1 2 3 2e^9 8 9 10];
        %       this will show up as 2 gaps. since I'm not really sure what
        %       is going on with that weirdly huge number, I'm going to
        %       replace that sample. 
        %       num_gaps = 2; 
        %       iGap = [3 4];
        %       nMissingSamp = (8-3)-1 = 4; 
        %       xVals = [t_amplifier(iGap(1)) t_amplifier(iGap(2)+1)]; 
        %       yVals = [amplifierData(ch, iGap(1)) amplifierData(ch, iGap(2)+1)];
        %       wantVals =
        %       [t_amplifier(iGap(1))+1:1:t_amplifier(iGap(2)+1)-1]; 
        %       interpVals = interp1(xVals, yVals, wantVals); 
        %--------------------- end brainstorm -----------------------
        
        iGap = find(diff(t_amplifier) ~= 1); 
        if num_gaps == 1 %scenario 1
            if (t_amplifier(iGap+1) - t_amplifier(iGap)) > 1000
                error('This is a really big gap. You should examine further')
            end
            iGapEdge = [iGap iGap+1];
            scenario = 1;
        elseif num_gaps == 2 %scenario 2
            if (t_amplifier(iGap+1)-t_amplifier(iGap)) < 1000
                error('this is a gap situation you havent dealt with yet. you should examine further')
            end
            iGapEdge = [iGap(1) iGap(2)+1];
            scenario = 2; 
        elseif num_gaps > 2
            error('warning, greater than 2 gaps detected. you need to write in a way to deal with this situation')
        end
        
        nMissedSamp = (t_amplifier(iGapEdge(2)) - t_amplifier(iGapEdge(1))) - 1;
        
        %%% print out info about the gap and force acknowledgement
        fprintf('the gap is %d samples, which is %d ms\n', nMissedSamp, nMissedSamp/sample_rate*1000)
        toContinue = input('To continue and correct the gap with linear interpolation, enter 1. Else, enter 0.  '); 
        if ~toContinue
            error(['you chose not to continue with linear interpolation for file ', filename])
        end
        toPlot = input('To visualize the correction across all channels, enter 1. To skip plotting, enter 0.  ');
        
        %%% get the values that you want to do the interpolation around.
        % this is dependent on the scenario. 
        % xVals and wantVals are the same across all data types and
        % channels
        % scenario 1: add samples only
        % scenario 2: remove one sample (the very large t), add missing samples 
        xVals = [t_amplifier(iGapEdge(1)) t_amplifier(iGapEdge(2))];
        wantVals = t_amplifier(iGapEdge(1))+1:1:t_amplifier(iGapEdge(2))-1;
        newSize = size(t_amplifier,2) + nMissedSamp - (scenario-1); 
        if length(wantVals) ~= nMissedSamp
            error('issue with calculating the sampleIDs of the missed samples')
        end
        
        %%% set some var for plotting
        xx1 = iGapEdge(1)-20:iGapEdge(1)+nMissedSamp+19;
        xx2 = iGapEdge(1)+1:1:iGapEdge(1)+nMissedSamp;
        xx3 = [iGapEdge(1) iGapEdge(2)+nMissedSamp-(scenario-1)];
        
        %%% ----------- do the interpolation %ALP 3/11/21
        %%% amplifier data
        gap_amplifier_data = amplifier_data;
        interp_amplifier_data = NaN(size(gap_amplifier_data,1), newSize);
        for ch = 1:size(gap_amplifier_data,1)
            yVals = [gap_amplifier_data(ch, iGapEdge(1)) gap_amplifier_data(ch, iGapEdge(2))];
            interpVals = interp1(xVals, yVals, wantVals);
            interp_amplifier_data(ch,:) = [gap_amplifier_data(ch,1:iGapEdge(1)) interpVals gap_amplifier_data(ch,iGapEdge(2):end)];
            
            %set some plotting stuff based on the scenario
            if toPlot
                figure
                hold on
                plot(xx1, interp_amplifier_data(ch, xx1))
                plot(xx2,interpVals, 'r-')
                plot(xx3, yVals, 'g*')
                legend('amplifier data', 'interp values', 'gapEdge')
                xlabel('sample')
                ylabel('uV')
                title(['AMPLIFIER DATA ', num2str(nMissedSamp) ' sample gap. ', num2str(nMissedSamp/sample_rate*1000),' ms. scenario ', num2str(scenario)])
                pause
            end
            %%% end plot to check
            clear yVals interpVals
        end
        
        %%% adc_data
        gap_adc_data = board_adc_data;
        interp_adc_data = NaN(size(gap_adc_data,1), newSize); 
        for ch = 1:size(board_adc_data)
            yVals = [gap_adc_data(ch, iGapEdge(1)) gap_adc_data(ch, iGapEdge(2))];
            interpVals = interp1(xVals, yVals, wantVals);
            interp_adc_data(ch,:) = [gap_adc_data(ch,1:iGapEdge(1)) interpVals gap_adc_data(ch,iGapEdge(2):end)]; 
        
            if toPlot
            figure
            hold on
            plot(xx1, interp_adc_data(ch, xx1))
            plot(xx2,interpVals, 'r-')
            plot(xx3, yVals, 'g*')
            legend('adc data', 'interp values', 'gapEdge')
            xlabel('sample')
            ylabel('V')
            title(['ADC DATA ', num2str(nMissedSamp) ' sample gap. ', num2str(nMissedSamp/sample_rate*1000),' ms. scenario ', num2str(scenario)])
            pause
            end
            clear yVals interpVals
        end
        ...
        %%% dig_in_data
        gap_dig_in_data = board_dig_in_data;
        interp_dig_in_data = NaN(size(gap_dig_in_data,1), newSize);
        for ch = 1:size(gap_dig_in_data,1)
            yVals = [gap_dig_in_data(ch, iGapEdge(1)) gap_dig_in_data(ch, iGapEdge(2))];
            interpVals = interp1(xVals, yVals, wantVals);
            interp_dig_in_data(ch,:) = [gap_dig_in_data(ch,1:iGapEdge(1)) interpVals gap_dig_in_data(ch,iGapEdge(2):end)];
            
            if toPlot
                figure
                hold on
                plot(xx1, interp_dig_in_data(ch, xx1))
                plot(xx2,interpVals, 'r-')
                plot(xx3, yVals, 'g*')
                legend('dig in data', 'interp values', 'gapEdge')
                xlabel('sample')
                ylabel('V')
                title(['DIGIN DATA ', num2str(nMissedSamp) ' sample gap. ', num2str(nMissedSamp/sample_rate*1000),' ms. scenario ', num2str(scenario)])
                pause
            end
            clear yVals interpVals
        end
        ...
        %clear original data structures, replace with interpolated ones
        amplifier_data = []; board_dig_in_data = []; board_adc_data = []; 
        amplifier_data = interp_amplifier_data;
        board_dig_in_data = interp_dig_in_data; 
        board_adc_data = interp_adc_data; 
        
        %correct the t_amplifier vector 
        new_t_amplifier = t_amplifier(1:iGapEdge(1));
        new_t_amplifier = [new_t_amplifier t_amplifier(iGapEdge(1))+1:1:t_amplifier(iGapEdge(2))-1];
        new_t_amplifier = [new_t_amplifier t_amplifier(iGapEdge(2):end)]; 
        
        if (length(new_t_amplifier) ~= size(amplifier_data,2)) || (sum(diff(new_t_amplifier) > 1) > 0)
            error('t_amplifier correction did not work properly')
        end
        t_amplifier = []; 
        t_amplifier = new_t_amplifier; 
        
        %check that the final size is what we expect based off of the
        %scenario and the number of missing samples 
        if size(amplifier_data,2) ~= size(board_dig_in_data,2) || size(amplifier_data,2) ~= size(board_adc_data,2)
            error('data sizes are not the same')
        end
        if size(amplifier_data,2) ~= newSize
            error('output data size is not what was expected.')
        end
        
        %set stuff for gap info that I might want later
        gapInfo.isGap = isGap; 
        gapInfo.nMissingSamp = nMissedSamp; 
        gapInfo.iGapStart = iGapEdge(1); 
        gapInfo.file = filename; 
    end
...
% Put File Info stuff in this Structure.
intandata.fileinfo = struct( ...
...
intandata.board_dig_in_data = board_dig_in_data; 
...
% intandata.fileinfo.num_amplifier_channels;
% intandata.fileinfo.num_aux_input_channels;
% intandata.fileinfo.num_supply_voltage_channels;
% intandata.fileinfo.num_board_adc_channels;
% intandata.fileinfo.num_board_dig_in_channels;
% intandata.fileinfo.num_board_dig_out_channels;

% Move variables to base workspace. ALP 03/21/17 save variables directly
